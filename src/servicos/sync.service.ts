...
export const syncService = {
  ...
  async syncAll(): Promise<{
    ...
    if (this.isSyncing) {
      for (let attempt = 0; attempt < 300 && this.isSyncing; attempt += 1) {
      ...
      if (this.isSyncing) {
        return {
          ok: false,
          count: 0,
          message: "A sincronização anterior ainda está em andamento.",
        };
      }
    }
    ...
    try {
      ...
      for (const item of items) {
        ...
        try {
          ...
          await localdb.outbox.update(item.id!, {
            status: "error",
            errorMessage: error instanceof Error ? error.message : "Falha ao enviar este item.",
          });
        }
      }
      ...
      const remaining = await pendingOutbox(ctx.farm_id);
      return remaining.length > 0
        ? {
            ok: false,
            count,
            message: `${remaining.length} item(ns) ainda aguardam sincronização. Toque para tentar novamente.`,
          }
        : { ok: true, count };
    } finally {
